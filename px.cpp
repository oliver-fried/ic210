...
int main()
{
    ...
    // these track if we should play another round
    // if there's a tie, AND if we should play another
    // game if the user wants to.
    ...
    // which allows us to track if they stand in
    // succession
    ...
    // each round the player must bet 4
    ...
    while(true){
        ...
        // if first run, allow player to exit
        if(i == 1){
            cout << "hit, stand or withdraw ? [h/s/w] ";
        }
        ...
        // if they stand, set bool to true
        else if(option == 's') {
            didPlayerStand = true;
        }

        // if they withdrawl, remove half money bet
        else if(option == 'w') {
            totalCash = totalCash + 2;
            keepPlaying = false;
            showHands(playerHand, dealerHand, true);
            cout << "Player withdraws" << endl;
            break;
        }
    ...
    cout << "You have $" << totalCash << endl;

    // reshuffle!
    fullCardDeck = shuffleDeck(cardDeck(), seed);
    }

    // if no money left to play again, stop game totally
    if(totalCash < 4) {
        cout << "GAME OVER: Not enough money" << endl;
        playAnother = false;
    }
    ...
    //otherwise, ask to play again
        ...
        cin >> playAgain;
        // if so, set the necessary conditions
        if(playAgain == 'y'){
            playAnother = true;
            keepPlaying = true;
        }
